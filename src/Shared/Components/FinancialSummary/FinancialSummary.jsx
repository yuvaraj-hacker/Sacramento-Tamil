            ...
            <section className="max-w-[50rem] mx-auto px-5 my-10">
                {accordionItems.map((item) => (
                    <div key={item.id} className=" ">
                        <button
                            className="flex items-center w-full justify-between bg-gray-100 border-b-2 p-2"
                        ...
                        <div
                            className={`overflow-hidden transition-all duration-500 ease-in-out ${openAccordions[item.id] ? 'max-h-96' : 'max-h-0'
                                }`}
                        >
                            {openAccordions[item.id] && (
